...
Colour.prototype.set = function(col){
	...
	if( (c.indexOf('#') == 0) && (c.length == 4) ){			//short hex
		colarray = this.getShortHex(c);
	}
	else if( (c.indexOf('#') == 0) && (c.length == 7) ){	//hex
		colarray = this.getHex(c);
	}
	else if(c.indexOf('rgb(') == 0){						//rgb
		colarray = this.getRgb(c);
	}
	else if(c.indexOf('rgba(') == 0){						//rgba
		colarray = this.getRgba(c);
	}
	else if(c.indexOf('hsl(') == 0){						//hsl
		colarray = this.getHsl(c);
	}
	else if(c.indexOf('hsla(') == 0){						//hsla
		colarray = this.getHsla(c);
	}
	else{													//keyword
		colarray = this.getName(c);
	}
...
// a colour in any standard web colour format
Colour.prototype.compare = function(col2){
	// Convert input to string, shift to lower case and remove white-space
	...
	// Decide what format the input is in an pass to the appropriate function
	if( (c.indexOf('#') == 0) && (c.length == 4) )	{colarray = this.getShortHex(c);}	//short hex
	else if( (c.indexOf('#') == 0) && (c.length == 7) )	{colarray = this.getHex(c);}		//hex
	else if(c.indexOf('rgb(') == 0)		{colarray = this.getRgb(c);}							//rgb
	else if(c.indexOf('rgba(') == 0)	{colarray = this.getRgba(c);}						//rgba
	else if(c.indexOf('hsl(') == 0)		{colarray = this.getHsl(c);}							//hsl
	else if(c.indexOf('hsla(') == 0)	{colarray = this.getHsla(c);}						//hsla
	else 								{colarray = this.getName(c);}						//keyword
...
//__________________________________________________________________
// hsla
Colour.prototype.hsla = function(){
	var red = this.r / 255.0;
	var green = this.g / 255.0;
	var blue = this.b / 255.0;
...
//__________________________________________________________________
// name
Colour.prototype.name = function(){
	if( (this.r == 0) && (this.g == 0) && (this.b == 0) && (this.a == 0.0) ) return 'transparent';
	
	for(var i = 0; i != this.keywords.length; ++i)
	{
		if( (this.keywords[i][1] == this.r) && (this.keywords[i][2] == this.g) && (this.keywords[i][3] == this.b) )
		{
			return this.keywords[i][0];
		}
	}
...
//__________________________________________________________________
// name
Colour.prototype.getName = function(c){
	if(c == 'transparent') return [0,0,0,0.0];
	
	for(var i = 0; i != this.keywords.length; ++i)
	{
		if(this.keywords[i][0] == c)
		{
			return [this.keywords[i][1], this.keywords[i][2], this.keywords[i][3], 1.0];
		}
	}
...
//__________________________________________________________________
// hex
Colour.prototype.getHex = function(c){
	var red = c.substring(1,3);
	var green = c.substring(3,5);
	var blue = c.substring(5,7);
	
	red = this.hex2dec(red);
	green = this.hex2dec(green);
	blue = this.hex2dec(blue);
	...
	//__________________________________________________________________
	// short hex	
Colour.prototype.getShortHex = function(c){
	var red = c.charAt(1);
...
//__________________________________________________________________
// rgb
Colour.prototype.getRgb = function(c){
...
//__________________________________________________________________
// rgba
Colour.prototype.getRgba = function(c){
...
//__________________________________________________________________
// hsl
Colour.prototype.getHsl = function(c){
	...
	//__________________________________________________________________
	// hsla
Colour.prototype.getHsla = function(c){
	// Define a sub-function used three times within getHsl
	function hue2rgb(m1,m2,h)
	{
		if(h < 0.0) h += 1.0;
...
Colour.prototype.isValid = function(){
	return this.valid;
}
...
Colour.prototype.dec2hex = function(d){
	var h = d.toString(16);
	if(h.length == 1) h = 0 + h;
	return h;
}
...
Colour.prototype.hex2dec = function(h){
